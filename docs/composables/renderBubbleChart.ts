...
export const renderBubbleChart = (
  { canvas, data }: BubbleChartOptions
): void => {
  ...
  const datasets = Array.from(allItems).map((item) => {
    const itemData = Object.keys(data.executionTime).map(group => {
      const executionTime = data.executionTime[group][item] || 0;
      const heapUsage =
        data.heapUsage[group] && data.heapUsage[group][item]
        ...
        tooltip: {
          callbacks: {
            label: function(context: any) {
              ...
              const groupLabel = (context.raw as { groupLabel?: string }).groupLabel || 'N/A';
